...
CUSTOMER_BOOKING_LINK_PATTERN = re.compile(r"^/start(?:@[A-Za-z0-9_]+)?\s+bloombooking_([a-f0-9]{48})$")
...
def customer_booking_link_token(text: str | None) -> str | None:
    match = CUSTOMER_BOOKING_LINK_PATTERN.fullmatch((text or "").strip())
    return match.group(1) if match else None
...
async def connect_customer_booking_notifications(
    *, public_url: str, bot_token: str, link_token: str, chat_id: int, telegram_user_id: int
) -> dict[str, Any]:
    if not public_url.startswith("https://"):
        raise BookingNotificationError("Адрес Bloom Online настроен некорректно.")
    if chat_id <= 0 or chat_id != telegram_user_id:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.post(
                f"{public_url.rstrip('/')}/api/telegram/customer-link",
                headers={"Authorization": f"Bearer {bot_token}"},
                json={"token": link_token, "chatId": str(chat_id)},
            )
    except httpx.HTTPError as exc:
        raise BookingNotificationError("Bloom Online временно недоступен. Попробуйте ещё раз.") from exc
    try:
    ...
    if not isinstance(appointment, dict):
        raise BookingNotificationError("Bloom Online не вернул данные записи.")
    return appointment
