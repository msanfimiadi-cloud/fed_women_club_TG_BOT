...
async def start(message: Message, state: FSMContext, settings: Settings) -> None:
    await state.clear()
    customer_token = customer_booking_link_token(message.text)
    if customer_token:
        await handle_customer_booking_notification_link(message, settings, customer_token)
        return
...
async def handle_customer_booking_notification_link(message: Message, settings: Settings, token: str) -> None:
    user = message.from_user
    if user is None or getattr(message.chat, "type", "") != "private":
        await message.answer("Подключите уведомления в личном диалоге с ботом.")
        return
    ...
    if not public_url:
        ...
        return
    try:
        appointment = await connect_customer_booking_notifications(
            public_url=public_url,
            bot_token=settings.telegram_bot_token,
            link_token=token,
            chat_id=message.chat.id,
        ...
        return
    await message.answer(
        "✅ <b>Уведомления о записи подключены</b>\n\n"
        f"<b>Салон:</b> {escape(str(appointment.get('organizationName') or ''))}\n"
        f"<b>Услуга:</b> {escape(str(appointment.get('serviceName') or ''))}\n"
        f"<b>Мастер:</b> {escape(str(appointment.get('staffName') or ''))}\n"
        f"<b>Дата:</b> {escape(str(appointment.get('date') or ''))} в {escape(str(appointment.get('time') or ''))}\n"
        f"<b>Стоимость:</b> {escape(str(appointment.get('price') or '0'))} ₽\n"
        f"<b>Адрес:</b> {escape(str(appointment.get('address') or ''))}\n\n"
        f'<a href="{escape(str(appointment.get("manageUrl") or ""))}">Управление записью</a>\n\n'
        "Сюда придут уведомления о переносе, отмене и изменении статуса записи."
    )
