...
- `/start` — показать новым участницам приветствие, информацию о 15 бесплатных днях,
  инструкцию по входу и добавлению приложения на экран телефона, а также ссылки Bloom Club.
...
- `➕ Создать партнёра`
- `📋 Список партнёров`
- `🤝 Выдать права партнёра` — добавить Telegram ID получателя уведомлений о новых пользователях.
- `📣 Добавить рекламодателя` — создать персональную рекламную ссылку по Telegram ID и имени.
- `📊 Статистика рекламы` — посмотреть результаты каждого рекламодателя и общий итог.
- `🎁 Создать розыгрыш`
- `📋 Список розыгрышей`
...
Когда пользователь впервые запускает бота командой `/start`, все администраторы и
назначенные партнёры получают его имя, Telegram username, Telegram ID и ссылку на
профиль. Повторный запуск тем же пользователем не создаёт повторное уведомление.

Чтобы назначить партнёра, администратор нажимает `🤝 Выдать права партнёра` и
отправляет числовой Telegram ID. Такой партнёр получает только уведомления и не
получает доступ к административному меню. Назначения и первые запуски сохраняются
в `admin_bot/data/bot_notifications.sqlite3` и не теряются при перезапуске бота.

## Рекламодатели, ссылки и статистика

Администратор нажимает `📣 Добавить рекламодателя`, отправляет Telegram ID и имя
рекламодателя. Бот создаёт постоянную персональную ссылку вида
`https://t.me/<имя_бота>?start=ad_<уникальный_код>` и отправляет её администратору
и рекламодателю. Если рекламодатель ещё не запускал бота, попросите его сначала
открыть личный диалог и нажать `/start`.

При первом переходе нового пользователя по рекламной ссылке администраторы и
назначенные партнёры видят имя рекламодателя, ссылку и данные пользователя.
Сам рекламодатель получает уведомления только о пользователях, пришедших по его
собственной ссылке. Повторный `/start`, переход уже известного пользователя и
переход рекламодателя по своей ссылке не увеличивают статистику.

Администратор открывает `📊 Статистика рекламы`, чтобы увидеть показатели всех
рекламодателей. Рекламодатель нажимает `📊 Моя статистика` или отправляет `/stats`
и видит только собственные переходы: за сегодня, 7 дней, 30 дней и всё время.

## Защита кодов входа

Для каждого Telegram-профиля разрешено не больше пяти запросов кода за 15 минут.
После исчерпания лимита бот показывает оставшееся время ожидания. Попытки
сохраняются в SQLite и не сбрасываются при перезапуске бота. Проверку вводимых
кодов на сайте отдельно ограничивает сервер Bloom Club.
